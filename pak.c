...
bool_t pak_read_dir(restable_t * rt)
{
  ...
  while (1)
  {
    if (readf(rt->file, "l4", &offset) != OK)
    {
      fprintf(stderr, "pak_read_dir: Can't read offset of entry #%zu.\n", i);
      return FALSE;
    }
    if (offset == 0)
      break;
    if ((offset < prev) || (offset > size))
    {
      fprintf(stderr, "pak_read_dir: Not ascending offset of entry #%zu.\n", i);
      return FALSE;
    }
    prev = offset;
    if (rt_add_entry(rt) == FALSE)
    {
      fprintf(stderr, "pak_read_dir: Can't add entry #%zu to dir.\n", i);
      return FALSE;
    }
    if (readf(rt->file, "sz", &(rt->entries[i].name)) != OK)
    {
      fprintf(stderr, "pak_read_dir: Can't read name of entry #%zu.\n", i);
      return FALSE;
    }
  ...
  if (rt->number > 0)
  {
    rt->entries[rt->number - 1].size =
      fsize(rt->file) - rt->entries[rt->number - 1].offset;
