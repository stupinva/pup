...
bool_t s_strtoul(const char *s, size_t *number)
{
	char *p;
...
bool_t parse_pair(const char *field, const char *value, size_t line)
{
	...
	if (strcmp(field, "type") == 0)
	{
		...
		{
			fprintf(stderr, "parse_pair warning: Value of type is not a number, line #%zu was skipped.\n", line);
			return FALSE;
		}
		type_set = TRUE;
	}
    else if (strcmp(field, "id") == 0)
    {
		...
		{
			fprintf(stderr, "parse_pair warning: Value of id is not a number, line #%zu was skipped.\n", line);
			return FALSE;
		}
		id_set = TRUE;
	}
	...
	{
		fprintf(stderr, "parse_pair warning: Unknown field name \"%s\", line #%zu was skipped.\n", field, line);
		return FALSE;
	}
	return TRUE;
}
...
bool_t rt_load_meta(restable_t *rt)
{
	...
	for(line = 0; ; line++)
	{
		...
		if (err != OK)
		{
			fprintf(stderr, "rt_load_meta: Fatal error on line #%zu, tail of file was skipped.\n", line);
			break;
		}
		if (term == EOF)
			break;
		...
		if (term != ':')
		{
			fprintf(stderr, "rt_load_meta warning: Syntax error on line #%zu, line was skipped.\n", line);
			continue;
		}
