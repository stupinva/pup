...
bool_t pack_read_dir(restable_t * rt)
{
  ...
  for(i = 0; i < rt->number; i++)
  {
    ...
    {
      fprintf(stderr, "pack_read_dir: Can't read entry #%zu.\n", i);
      return FALSE;
    }
...
bool_t pack_write_dir(restable_t * rt)
{
  ...
  if (writef
      (rt->file, "c4l4l4", PACK_IDENT, offset,
       rt->number * PACK_ENTRY_SIZE) != OK)
  {
  ...
  for(i = 0; i < rt->number; i++)
  {
    if (writef(rt->file, "s56l4l4",
               ...
               rt->entries[i].offset, rt->entries[i].size) != OK)
    {
      fprintf(stderr, "pack_write_dir: Can't write entry #%zu.\n", i);
      return FALSE;
    }
  }
  return TRUE;
