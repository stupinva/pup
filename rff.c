...
void rff_pack_name(char *resname, char *resext, const char *nameext)
{
  ...
  memset(resname, 0, RFF_ENTRY_NAME_SIZE);
  memset(resext, 0, RFF_ENTRY_EXT_SIZE);
...
bool_t rff_fextract(const char *dst_filename, FILE * src, size_t size,
                    bool_t decrypt)
{
  FILE *dst;
  int ch;
...
bool_t rff_fadd(FILE * dst, const char *src_filename, size_t * psize,
                bool_t encrypt)
{
  FILE *src;
  int ch;
...
bool_t rff_add_resource(restable_t * rt, size_t i)
{
  bool_t encrypt;
  char *filename;
...
bool_t rff_extract_resource(restable_t * rt, size_t i)
{
  char *filename;
  bool_t decrypt;
  ...
  {
    fprintf(stderr,
            "rff_extract_resource: Can't create or write file \"%s\".\n",
            filename);
    s_free(&filename);
    return FALSE;
  }
...
bool_t rff_read_dir(restable_t * rt)
{
  ...
  if (readf(rt->file, "c4l4l4l4c16", ident, &version, &offset, &number, NULL) !=
      OK)
  {
  ...
  for(i = 0; i < rt->number; i++)
  {
    ...
    unpackf(&buf, "c16l4l4c4l4l1c3c8l2c2",
            ...
            NULL,
            &(rt->entries[i].time),
            &(rt->entries[i].type), ext, name, &(rt->entries[i].id), NULL);
...
bool_t rff_write_dir(restable_t * rt, size_t version)
{
  size_t offset;
  unsigned char key;
  ...
  if (writef
      (rt->file, "c4l4l4l4c16", RFF_IDENT, version, offset, rt->number,
       NULL) != OK)
  {
