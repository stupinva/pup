...
bool_t viv_read_dir(restable_t * rt)
{
  ...
  for(i = 0; i < rt->number; i++)
  {
    if (readf(rt->file, "b4b4sz",
              &(rt->entries[i].offset),
              &(rt->entries[i].size), &(rt->entries[i].name)) != OK)
    {
      fprintf(stderr, "viv_read_dir: Can't read entry #%zu.\n", i);
      return FALSE;
    }
...
bool_t viv_prepare_dir(restable_t * rt)
{
  ...
  for(i = 0; i < rt->number; i++)
    offset +=
      VIV_ENTRY_OFFSET_SIZE + VIV_ENTRY_SIZE_SIZE +
      strlen(rt->entries[i].name) + 1;
...
bool_t viv_write_dir(restable_t * rt)
{
  ...
  if (writef(rt->file, "c4b4b4b4",
             VIV_IDENT, offset, rt->number, align(size, VIV_PAGE)) != OK)
  {
  ...
  for(i = 0; i < rt->number; i++)
  {
    if (writef(rt->file, "b4b4sz",
               ...
               rt->entries[i].size, rt->entries[i].name) != OK)
    {
      fprintf(stderr, "viv_write_dir: Can't write entry #%zu.\n", i);
      return FALSE;
    }
  }
