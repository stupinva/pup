...
ssize_t rt_search_by_filename(restable_t *rt, const char *filename, bool_t same_case)
{
	size_t i;
...
bool_t rt_fix(restable_t *rt)
{
	...
	for(i = 0; i < rt->number - 1; i++)
	{
		...
		fprintf(stderr, "Checking entries %zu/%zu. Found %zu referenced to same block, %zu with same content, %zu entries with same name.\r",
			i, rt->number, hardlinks, copies, renamed);
	}
	fprintf(stderr, "Checking entries finished. Found %zu referenced to same block, %zu with same content, %zu entries with same name.\n",
...
bool_t rt_search_equal_files(restable_t *rt)
{
	...
	for(i = 0; i < rt->number - 1; i++)
	{
		...
		fprintf(stderr, "Searching of files with same content %zu/%zu. Found %zu files.\r", i, rt->number, copies);
	}
	fprintf(stderr, "Searching of files with same content finished. Found %zu files.\n", copies);
	s_free(&filename0);
	s_free(&filename1);
	return TRUE;
}
