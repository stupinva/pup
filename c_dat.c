...
bool_t dat_decompress(unsigned char *sdata, size_t ssize, unsigned char *ddata,
                      size_t dsize)
{
  size_t soffset;
  size_t doffset;
  ...
  for(;;)
  {
    ...
    else if ((n & 0x8000) == 0)
    {
      ...
      if (dblock > BLOCKSIZE)
        dblock = BLOCKSIZE;
      if (lzss_decompress(&(sdata[soffset]), sblock, &(ddata[doffset]), dblock)
          == FALSE)
      {
...
bool_t dat_compress(unsigned char *sdata, size_t ssize, unsigned char *ddata,
                    size_t * dsize0, int level)
{
  ...
  for(;;)
  {
    ...
    if (lzss_compress
        (&(sdata[soffset]), sblock, &(ddata[doffset + 2]), &dblock,
         level) == TRUE)
      if (dblock < sblock)
      {
