...
bool_t vpp_read_dir(restable_t * rt)
{
  ...
  for(i = 0; i < rt->number; i++)
  {
    if (readf(rt->file, "s60l4",
              &(rt->entries[i].name), &(rt->entries[i].size)) != OK)
    {
      fprintf(stderr, "vpp_read_dir: Can't read entry #%zu.\n", i);
      return FALSE;
    }
...
{
  fseek(rt->file, VPP_PAGE + align(rt->number * VPP_ENTRY_SIZE, VPP_PAGE),
        SEEK_SET);
  return TRUE;
}
...
bool_t vpp_write_dir(restable_t * rt)
{
  ...
  if (writef(rt->file, "l4l4l4l4", VPP_IDENT, VPP_VERSION, rt->number, offset)
      != OK)
  {
  ...
  for(i = 0; i < rt->number; i++)
  {
    if (writef(rt->file, "s60l4",
               rt->entries[i].name, rt->entries[i].size) != OK)
    {
      fprintf(stderr, "vpp_write_dir: Can't write entry #%zu.\n", i);
      return FALSE;
    }
  }
