  ...
  if (level != 0)
  {
    *compressed = comdec->dsize(*size);
    ...
    if (d != NULL)
    {
      ...
      {
        fprintf(stderr,
                "c_fadd: compression failed, trying to add uncompressed data.\n");
        free(d);
        *compressed = *size;
      }
      ...
      {
        fprintf(stderr,
                "c_fadd: compression not effective, trying to add uncompressed data.\n");
        free(d);
        *compressed = *size;
      }
      else
    ...
    {
      fprintf(stderr,
              "c_fadd: can't get memory for compression, trying to add uncompressed data.\n");
      *compressed = *size;
    }
  }
  else
